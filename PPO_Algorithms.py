...
def policy(state):
	...
	# old_prob = tf.math.log( p_action[action] )
		...
		for i in range( 3 ):
			...
			for j in range(5):
				...
				with tf.GradientTape() as tape:
					...
					# concat = tf.concat([first_term , second_term],-1)
					# policy_loss = -tf.math.reduce_mean( tf.math.reduce_min( concat , -1 , keepdims=True ) )
					policy_loss = tf.math.minimum( first_term , second_term )
					policy_loss = -1.0*tf.math.reduce_mean( policy_loss + 0.2 * ( new_prob * tf.math.log(new_prob + 1e-10)) )
